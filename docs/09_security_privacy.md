...
| GitHub リフレッシュトークン | 同 `github_refresh_token` | 期限付きトークンのときだけ保存（04 §1.3b） |
...
- トークンの削除は、GitHubが実際にトークンを拒否し、**かつ更新もできなかった**場合に限る。トークンが手元に無い状態で起きた認証失敗では削除しない。
- 認証の出来事は `key_value` の `auth_log` に残す（最新30件、設定画面から閲覧可）。記録してよいのは時刻・種別・`expires_in` の有無・トークンの先頭8文字のハッシュまで。トークン本体・リフレッシュトークンは書かない。数時間後に起きるサインアウトは再現が難しく、この記録以外に手がかりが無い。
...
GitHubへの書き込みには2種類ある。ファイルの変更は保留中の変更に溜め、ユーザーが明示的にコミットするまで送信しない。一方、スレッドへのコメント投稿とリアクション（FR-98, FR-99）は、ボタンを押した時点で即座にGitHubへ送信される。取り消せるのはリアクションだけで、もう一度押すと消える。通信先は `api.github.com`（REST と GraphQL）と、設定で選んだAI提供元のAPIのみ。
